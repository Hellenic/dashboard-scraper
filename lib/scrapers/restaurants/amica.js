...
let AmicaScraper = {
  ...
  doScrape() {
    ...
    request(RESTAURANT_BASE_URL + amicaRestaurantsURL, function(error, response, body) {
      ...
      restaurantDatas.filter(function(r) { return r.Type == 'restaurant'; }).forEach(r => {
        sleep.sleep(1);
        ...
        osmosis
          ...
          .data(function(partial) {
            ...
            request.post({url: BASE_URL, form: restaurant}, function(err, httpResponse, body) {
              ...
              costNumber = 3542;
              let firstDay = '2015-12-21', lastDay = '2015-12-26'; // Optional params
...
function parseMenus(MenusForDays)
{
  ...
  MenusForDays.map(function(daymenu) {
    ...
    let times = daymenu.LunchTime.split('-'); // Format: 10.30 - 13.00 or Klo 10.45 - 13.00
    ...
    daymenu.SetMenus.map(function(menu) {
      ...
      var menuitem = {
        ...
        validTo: validToDate,
        price: menu.Price,
        properties: menu.Components.join(', ')
      };
